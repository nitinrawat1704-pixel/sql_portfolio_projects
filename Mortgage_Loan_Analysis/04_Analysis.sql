...
-- 55 Calculate the approval rate for each credit-score band:
...
concat(round((total_approved_application/total_application)*100),"%") as approval_rate from cte;

-- 56 High-Risk Applications
with cte as(
select
customer_name,
application_id,
...
select customer_name,application_id,credit_score,LTV,DTI,application_status FROM CTE Where LTV >80 and DTI>40;

-- Q57 — Officer Quality vs Volume
select 
officer_name,
...
sum(loan_amount) as total_loan_amount_handled
from loan_application a inner join loan_officer o on a.officer_id=o.officer_id
group by 1
order by 4 desc;

-- Q58 - Region Performance
...
from loan_application;
 ...
 -- Q60 For every customer who has applied more than once, show:
 

with cte as(
select customer_name,
first_value(application_status)over(partition by c.customer_id ORDER BY application_date) first_status,
last_value(application_status)over(partition by c.customer_id ORDER BY application_date ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) last_status,
first_value(application_date)over(partition by c.customer_id) first_application_date ,
last_value(application_date)over(partition by c.customer_id) last_application_date,
row_number()over(partition by c.customer_id ORDER BY application_date) as rn
from loan_application a inner join customers c on a.customer_id=c.customer_id 
)
select customer_name,first_status,last_status,first_application_date,last_application_date from cte where rn=1 and  first_status <> last_status;

-- Q61 — Processing Time by Loan Type
...
-- Q62 Monthly Funding Performance
...
concat(round(((tla-pmtla)/pmtla)*100),"%") as mom_growth_percentage from cte;

-- Q63 Create a risk category for every application.
